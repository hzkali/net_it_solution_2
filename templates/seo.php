...
if ( netmix_is_on( netmix_get_theme_option( 'seo_snippets' ) ) ) {
	...
	<div class="structured_data_snippets">
		...
		<div itemscope itemprop="publisher" itemtype="//schema.org/Organization">
			...
			<?php
			$netmix_logo_image = netmix_get_logo_image();
			if ( ! empty( $netmix_logo_image['logo'] ) ) {
				?>
				<meta itemprop="logo" itemtype="//schema.org/ImageObject" content="<?php echo esc_url( $netmix_logo_image['logo'] ); ?>">
				<?php
			}
			?>
		</div>
		<?php
		if ( netmix_get_theme_option( 'show_author_info' ) != 1 || ! is_single() || is_attachment() || ! get_the_author_meta( 'description' ) ) {  
			?>
