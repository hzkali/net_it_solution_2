...
if ( !is_user_logged_in() ) {
	?><ul class="sc_layouts_login_menu sc_layouts_menu_nav sc_layouts_menu_no_collapse"><li class="menu-item"><a href="#trx_addons_login_popup" class="trx_addons_popup_link trx_addons_login_link "><?php
		?><span class="sc_layouts_item_icon sc_layouts_login_icon sc_icons_type_icons "></span><?php
		if (!empty($args['text_login'])) {
			?><span class="sc_layouts_item_details sc_layouts_login_details"><?php
				...
				if (!empty($rows[2])) {
					?><span class="sc_layouts_item_details_line2 sc_layouts_iconed_text_line3"><?php echo esc_html($rows[2]); ?></span><?php
				}
			?></span><?php
		}
	?></a></li></ul><?php
	...
	?><ul class="sc_layouts_login_menu sc_layouts_dropdown sc_layouts_menu_nav sc_layouts_menu_no_collapse">
		<li class="menu-item<?php if (!empty($args['user_menu'])) { echo ' menu-item-has-children'; } ?>">
			<a href="<?php
				...
				<span class="sc_layouts_item_icon sc_layouts_login_icon sc_icons_type_icons icon-user-1 trx_addons_icon-<?php echo empty($args['user_menu']) ? 'user-times' : 'user-alt'; ?>"></span><?php
				if (!empty($args['text_logout'])) {
					?><span class="sc_layouts_item_details sc_layouts_login_details"><?php
						...
						if (!empty($rows[2])) {
							?><span class="sc_layouts_item_details_line3 sc_layouts_iconed_text_line3"><?php echo esc_html($rows[2]); ?></span><?php
						}
					?></span><?php
				}
			?></a><?php 
			if (!empty($args['user_menu'])) {
				?><ul><?php
					...
					if (current_user_can('publish_posts')) {
						?><li class="menu-item trx_addons_icon-wpforms"><a href="<?php echo esc_url(trailingslashit(home_url('/'))); ?>wp-admin/post-new.php"><span><?php esc_html_e('New post', 'netmix'); ?></span></a></li><?php
						// Delimiter
						?><li class="menu-item menu-delimiter"></li><?php
					}
					do_action('trx_addons_action_login_menu_settings');
					// Settings
					?><li class="menu-item trx_addons_icon-cog"><a href="<?php echo esc_url(get_edit_user_link()); ?>"><span><?php esc_html_e('My profile', 'netmix'); ?></span></a></li><?php
					// Delimiter
					?><li class="menu-item menu-delimiter"></li><?php
					do_action('trx_addons_action_login_menu_logout');
					// Logout
					?><li class="menu-item trx_addons_icon-user-times"><a href="<?php echo esc_url(wp_logout_url(home_url('/'))); ?>"><span><?php esc_html_e('Logout', 'netmix'); ?></span></a></li><?php
