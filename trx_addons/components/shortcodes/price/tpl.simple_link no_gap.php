...
?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> 
	...
	foreach ($args['prices'] as $item) {
		...
		?><div class="sc_price_item sc_price_item_<?php
			...
			if (!empty($item['dark_item']) || !empty($item['dark_item'])) echo ' dark_item'; 
		...
		//Hover for bg image
			...
			if (!empty($item['image'])) {
				...
				if (!empty($item['image'])) {
					...
					?><div class="sc_price_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e('Icon', 'netmix'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			} else {
				...
				if (!empty($icon)) {
					...
					?><div class="sc_price_item_icon sc_price_item_icon_type_<?php echo esc_attr($item['icon_type']); ?> <?php echo esc_attr($icon); ?>"
						...
						if (!empty($svg)) {
							...
							?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e('Icon', 'netmix'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						} else {
			...
			?><div class="sc_price_item_info"><?php
				...
				if (!empty($item['title'])) {
					...
					?><h4 class="sc_price_item_title"><?php
				...
				if (!empty($item['description'])) {
					...
					?><div class="sc_price_item_description"><?php
						foreach ($item['description'] as $str) {
							?><span><?php echo wp_kses(trx_addons_parse_codes($str), 'netmix_kses_content'); ?></span><?php
						}
					?></div><?php
				}
				...
				if (!empty($item['price'])) {
					...
					?><div class="sc_price_item_price"><?php
						if (!empty($item['before_price'])) {
							?><span class="sc_price_item_price_before"><?php echo wp_kses(trx_addons_parse_codes($item['before_price']), 'netmix_kses_content'); ?></span><?php
						}if (count($parts) > 1 && $parts[1]!='') {
							?><span class="sc_price_item_price_decimals"><?php echo wp_kses($parts[1], 'netmix_kses_content'); ?></span><?php
						}
						?><span class="sc_price_item_price_value"><?php echo wp_kses($parts[0], 'netmix_kses_content'); ?></span><?php
						
						if (!empty($item['after_price'])) {
							?><span class="sc_price_item_price_after"><?php echo wp_kses(trx_addons_parse_codes($item['after_price']), 'netmix_kses_content'); ?></span><?php
						}
					?></div><?php
				}
				if (!empty($item['link']) && !empty($item['link_text'])) {
					?><a href="<?php echo esc_url($item['link']); ?>" class="<?php
							echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_price_item_simple_link sc_button sc_button_simple sc_button_size_normal  color_style_dark', 'sc_price', $args, $item));
					?>"<?php
			...
			if (!empty($item['link']) && empty($item['link_text'])) {
				?><a href="<?php echo esc_url($item['link']); ?>" class="<?php
						echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_price_item_simple sc_price_item_link_over', 'sc_price', $args, $item));
				?>"<?php
