...
if ( ! function_exists( 'netmix_customizer_get_css' ) ) {

	function netmix_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts = netmix_customizer_add_theme_fonts( $fonts );
...
CSS;
			$rez          = apply_filters( 'netmix_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			// Make theme-specific fonts rules
			$vars = netmix_customizer_add_theme_vars( $vars );
			extract( $vars );
...
CSS;
			$rez          = apply_filters( 'netmix_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars']  = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( netmix_get_sorted_schemes() ) : array( $scheme );
			...
			if ( count( $schemes ) > 0 ) {
				...
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = netmix_get_scheme_colors( $s );
					}
...
.scheme_dark input[type="checkbox"] + .wpcf7-list-item-label a{
	color: {$colors['alter_hover2']}
}
.scheme_dark input[type="checkbox"] + .wpcf7-list-item-label a:hover{
...
.sc_button:not(.sc_button_simple):not(.sc_button_bordered):not(.sc_button_bg_image).color_style_default_dark_hover:hover,
...
.ripple.color_style_link2:before,
.ripple.color_style_link2:after{
	border-color: {$colors['text_link2']};
}
...
.ripple.color_style_link3:after,
.ripple.color_style_link3:before{
	border-color: {$colors['text_link3']};
}
...
.ripple.color_color_style_dark:after,
.ripple.color_color_style_dark:before{
...
.ripple.color_style_link2:hover:after,
.ripple.color_style_link2:hover:before{
...
.ripple.color_style_link3:hover:after,
.ripple.color_style_link3:hover:before{
	border-color: {$colors['text_hover3']};
}
...
.ripple.color_style_dark:hover:before,
.ripple.color_style_dark:hover:after{
	border-color: {$colors['text_link']};
}
...
.woocommerce input.button.disabled:hover, .woocommerce input.button:disabled:hover, .woocommerce input.button[disabled]:disabled:hover {
	background: {$colors['alter_hover_07']} !important;
...
.scheme_self .widget.woocommerce a.button,
...
.scheme_self .widget.woocommerce a.button:hover,
...
.sticky .post_meta:hover, 
.sticky .post_meta_item:hover, 
.sticky .post_meta .vc_inline-link:hover, 
.sticky .post_meta_item a:hover, 
.sticky .post_info .post_info_item:hover, 
.sticky .post_info .post_info_item a:hover, 
.sticky .post_info_counters .post_meta_item:hover,
.sticky .post_meta_item:hover:before{
	color:{$colors['alter_link2']};
}
.sticky.post_item{
	color:{$colors['alter_text']};
...
.sc_layouts_row_type_narrow  .sc_layouts_item + .sc_layouts_item:after{
...
.sc_layouts_row_type_normal.sc_layouts_row_fixed .sc_layouts_item a:not(.sc_button):not(.button){
	color: {$colors['alter_dark']};
}
...
.netmix_tabs .netmix_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};.esg-filterbutton.selected
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.netmix_tabs .netmix_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.sheme_self.sidebar .netmix_tabs .netmix_tabs_titles li.ui-state-active a {
...
.post_item_single .post_meta_item.post_date:hover,
...
.comments_list .post_meta_item:hover:before{
	color: inherit;
}
...
.sticky .post_meta_item .socials_share .social_items,
.sticky .post_meta_item .social_items:before {
	background-color: {$colors['alter_bg_color']};
}
...
.widget_calendar td#today:hover:before,
.wp-block-calendar td#today:hover:before {
	background-color: {$colors['alter_link2']};
}
...
.scheme_self.sidebar .widget_calendar #prev a:before,
.scheme_self.sidebar .widget_calendar #next a:before {
	color: {$colors['text_link']};
}
...
.esg-nav-by-shortcode.netmix-bullets .esg-pagination .esg-filterbutton.selected{
	background: {$colors['text_hover']}!important;
}
.esg-nav-by-shortcode.netmix-bullets .esg-pagination .esg-filterbutton{
	background: rgba(255,255,255,0.1)!important;
}
.esg-nav-by-shortcode.netmix-bullets .esg-pagination .esg-filterbutton:hover{
...
CSS;
					...
					$rez            = apply_filters(
						'netmix_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'netmix_filter_prepare_css', $css_str, $remove_spaces );
	}
}
