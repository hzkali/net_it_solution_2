...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'netmix_trx_addons_theme_specific_components' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'netmix_trx_addons_theme_specific_components' );
	function netmix_trx_addons_theme_specific_components( $enable = false ) {
		return false;
	}
}

if ( ! function_exists( 'netmix_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'netmix_trx_addons_theme_specific_setup1', 1 );
	function netmix_trx_addons_theme_specific_setup1() {
		if ( netmix_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'netmix_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'netmix_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'netmix_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'netmix_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'netmix_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'netmix_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'netmix_trx_addons_widgets_list' );
		}
	}
}

if ( ! function_exists( 'netmix_trx_addons_theme_specific_setup' ) ) {
	add_action( 'after_setup_theme', 'netmix_trx_addons_theme_specific_setup' );
	function netmix_trx_addons_theme_specific_setup() {
		if ( netmix_exists_trx_addons() ) {
			if ( ! is_admin() ) {
				add_action( 'netmix_action_before_post_meta', 'netmix_trx_addons_action_before_post_meta' );
			}
		}
	}
}
...
// CV
if ( ! function_exists( 'netmix_trx_addons_cv_enable' ) ) {
	
	function netmix_trx_addons_cv_enable( $enable = false ) {
		// To do: return false if theme not use CV functionality
		return NETMIX_THEME_FREE
					? false     // Free version
					: true;     // Pro version
	}
}
...
// Demo mode
if ( ! function_exists( 'netmix_trx_addons_demo_enable' ) ) {
	
	function netmix_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'netmix_trx_addons_edd_themes_market_enable' ) ) {
	
	function netmix_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'netmix_trx_addons_api_list' ) ) {
	
	function netmix_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'netmix_trx_addons_cpt_list' ) ) {
	
	function netmix_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'netmix_trx_addons_sc_list' ) ) {
	
	function netmix_trx_addons_sc_list( $list = array() ) {
...
// Widgets
if ( ! function_exists( 'netmix_trx_addons_widgets_list' ) ) {
	
	function netmix_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'netmix_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'netmix_trx_addons_menu_cache' );
	function netmix_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'netmix_trx_addons_localize_script' ) ) {
	...
	function netmix_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = netmix_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'netmix_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'netmix_trx_addons_sc_type', 10, 2 );
	function netmix_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = netmix_array_merge( $list, netmix_get_list_blog_styles( false, 'sc' ) );
		}
		if ($sc == 'trx_sc_layouts_cart') {
			$list['modern'] = 'Modern';
		}
		if ($sc == 'trx_sc_price') {
			$list['simple_link'] = 'Simple link';
		}
		if ($sc == 'trx_sc_price') {
			$list['simple_link no_gap'] = 'Simple link No Gap';
		}
		if ($sc == 'trx_sc_price') {
			$list['feature'] = 'Feature';
		}
		if ($sc == 'trx_sc_button') {
			$list['icon_button'] = 'Icon button';
		}
		return $list;
	}
}
...
if ( ! function_exists( 'netmix_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'netmix_trx_addons_sc_prepare_atts', 10, 2 );
	function netmix_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = netmix_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = netmix_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'netmix_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'netmix_trx_addons_sc_atts', 10, 2 );
	function netmix_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'netmix_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'netmix_trx_addons_sc_map', 10, 2 );
	function netmix_trx_addons_sc_map( $params, $sc ) {
		...
		if ( in_array(
			...
			$params['params'][] = array(
				'param_name'  => 'scheme',
				'heading'     => esc_html__( 'Color scheme', 'netmix' ),
				'description' => wp_kses_data( __( 'Select color scheme to decorate this block', 'netmix' ) ),
				'group'       => esc_html__( 'Colors', 'netmix' ),
				'admin_label' => true,
				'value'       => array_flip( netmix_get_list_schemes( true ) ),
				'type'        => 'dropdown',
			);
		}
		// Param 'color_style'
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'netmix' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'netmix' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( netmix_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'netmix' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'netmix' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( netmix_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'netmix_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'netmix_trx_addons_sc_output', 10, 4 );
	function netmix_trx_addons_sc_output( $output, $sc, $atts, $content ) {

		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			if ( ! empty( $atts['scheme'] ) && ! netmix_is_inherit( $atts['scheme'] ) ) {
				$output = str_replace( 'class="sc_action ', 'class="sc_action scheme_' . esc_attr( $atts['scheme'] ) . ' ', $output );
			}
			if ( ! empty( $atts['color_style'] ) && ! netmix_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'netmix_trx_addons_sc_item_link_classes' ) ) {
	add_filter( 'trx_addons_filter_sc_item_link_classes', 'netmix_trx_addons_sc_item_link_classes', 10, 3 );
	function netmix_trx_addons_sc_item_link_classes( $class, $sc, $atts=array() ) {
		if ( 'sc_button' == $sc ) {
			if ( ! empty( $atts['color_style'] ) && ! netmix_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'netmix_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'netmix_trx_addons_sc_item_title_tag' );
	function netmix_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'netmix_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'netmix_trx_addons_sc_item_button_args', 10, 3 );
	function netmix_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'netmix_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'netmix_trx_addons_slider_title', 10, 2 );
	function netmix_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'netmix_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'netmix_trx_addons_sc_googlemap_styles' );
	function netmix_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'netmix' );
		return $list;
	}
}

// Show reactions in the single posts
if ( ! function_exists( 'netmix_trx_addons_action_before_post_meta' ) ) {
	
	function netmix_trx_addons_action_before_post_meta() {
		if ( netmix_exists_trx_addons() ) {
...
if ( ! function_exists( 'netmix_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'netmix_trx_addons_tiny_mce_style_formats' );
	function netmix_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'netmix' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'netmix' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'netmix_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'netmix_filter_allow_override_header_image', 'netmix_trx_addons_allow_override_header_image' );
	function netmix_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					netmix_is_team_page()
					|| netmix_is_cars_page()
					|| netmix_is_cars_agents_page()
					|| netmix_is_properties_agents_page()
					)
				? false
				: $allow;
	}
...
if ( ! function_exists( 'netmix_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'netmix_trx_addons_thumb_size', 10, 2 );
	function netmix_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
...
if ( ! function_exists( 'netmix_trx_addons_override_options_fields' ) ) {
	add_filter( 'trx_addons_filter_override_options_fields', 'netmix_trx_addons_override_options_fields', 10, 2 );
	function netmix_trx_addons_override_options_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'netmix' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'netmix' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
