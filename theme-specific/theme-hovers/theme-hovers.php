...
if ( ! function_exists( 'netmix_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'netmix_hovers_theme_setup3', 3 );
	function netmix_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		netmix_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'netmix' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'netmix' ) ),
					'std'     => 'fade',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'netmix' ),
						'slide_left'   => esc_html__( 'Slide from Left', 'netmix' ),
						'slide_right'  => esc_html__( 'Slide from Right', 'netmix' ),
						'slide_top'    => esc_html__( 'Slide from Top', 'netmix' ),
						'slide_bottom' => esc_html__( 'Slide from Bottom', 'netmix' ),
			
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'netmix' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'netmix' ) ),
					'std'      => 'icon',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'netmix' ),
					),
					'options'  => netmix_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'netmix_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'netmix_hovers_theme_setup9', 9 );
	function netmix_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'netmix_hovers_frontend_scripts', 1010 );
		add_action( 'wp_enqueue_scripts', 'netmix_hovers_frontend_styles', 1100 );
		add_action( 'wp_enqueue_scripts', 'netmix_hovers_responsive_styles', 2000 );
		add_filter( 'netmix_filter_localize_script', 'netmix_hovers_localize_script' );
		add_filter( 'netmix_filter_merge_scripts', 'netmix_hovers_merge_scripts' );
		add_filter( 'netmix_filter_merge_styles', 'netmix_hovers_merge_styles' );
		add_filter( 'netmix_filter_merge_styles_responsive', 'netmix_hovers_merge_styles_responsive' );
		add_filter( 'netmix_filter_get_css', 'netmix_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'netmix_hovers_frontend_scripts' ) ) {
	
	function netmix_hovers_frontend_scripts() {
		if ( netmix_is_on( netmix_get_theme_option( 'debug_mode' ) ) ) {
			$netmix_url = netmix_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $netmix_url ) {
				wp_enqueue_script( 'netmix-hovers', $netmix_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'netmix_hovers_frontend_styles' ) ) {
	
	function netmix_hovers_frontend_styles() {
		if ( netmix_is_on( netmix_get_theme_option( 'debug_mode' ) ) ) {
			$netmix_url = netmix_get_file_url( 'theme-specific/theme-hovers/theme-hovers.css' );
			if ( '' != $netmix_url ) {
				wp_enqueue_style( 'netmix-hovers', $netmix_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'netmix_hovers_responsive_styles' ) ) {
	
	function netmix_hovers_responsive_styles() {
		if ( netmix_is_on( netmix_get_theme_option( 'debug_mode' ) ) ) {
			$netmix_url = netmix_get_file_url( 'theme-specific/theme-hovers/theme-hovers-responsive.css' );
			if ( '' != $netmix_url ) {
				wp_enqueue_style( 'netmix-hovers-responsive', $netmix_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'netmix_hovers_merge_styles' ) ) {
	
	function netmix_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'netmix_hovers_merge_styles_responsive' ) ) {
	
	function netmix_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'netmix_hovers_localize_script' ) ) {
	
	function netmix_hovers_localize_script( $arr ) {
		$arr['button_hover'] = netmix_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'netmix_hovers_merge_scripts' ) ) {
	
	function netmix_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'netmix_hovers_add_icons' ) ) {
	function netmix_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php netmix_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			<div class="icons">
						...
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'netmix' ) : esc_html__( 'Buy now', 'netmix' ) ) : '' )
										. '</a>',
							$product
						)
					);
				...
				if ( 'shop_buttons' == $hover ) {
					if ( is_object( $args['cat'] ) ) {
						esc_html_e( 'View products', 'netmix' );
					} else {
						esc_html_e( 'Details', 'netmix' );
					}
				}
				?>
				</a>
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php netmix_show_layout($target); ?> aria-hidden="true" class="icon-eye-1"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php netmix_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php netmix_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php netmix_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'netmix_hovers_get_css' ) ) {
	
	function netmix_hovers_get_css( $css, $args ) {
