<div class="front_page_section front_page_section_blog<?php
	$netmix_scheme = netmix_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $netmix_scheme ) && ! netmix_is_inherit( $netmix_scheme ) ) {
		echo ' scheme_' . esc_attr( $netmix_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( netmix_get_theme_option( 'front_page_blog_paddings' ) );
?>"
		<?php
		$netmix_css      = '';
		$netmix_bg_image = netmix_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$netmix_anchor_icon = netmix_get_theme_option( 'front_page_blog_anchor_icon' );
	$netmix_anchor_text = netmix_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( netmix_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$netmix_css      = '';
			$netmix_bg_mask  = netmix_get_theme_option( 'front_page_blog_bg_mask' );
			$netmix_bg_color_type = netmix_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $netmix_bg_color_type ) {
				$netmix_bg_color = netmix_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$netmix_caption = netmix_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $netmix_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $netmix_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $netmix_caption, 'netmix_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$netmix_description = netmix_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $netmix_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $netmix_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $netmix_description ), 'netmix_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
				...
				if ( ! netmix_exists_trx_addons() ) {
					...
					netmix_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
				}
			}
			?>
			</div>
