...
jQuery( document ).ready(
	function() {
		...
		netmix_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="netmix_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.netmix_post_editor' );
					if ( link.length > 0 ) {
						netmix_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.netmix_post_editor', function(e) {
					netmix_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('netmix_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function netmix_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
