...
(function() {
	...
	jQuery( document ).on(
		'action.ready_netmix', function() {

			// Change display mode
			jQuery( '.woocommerce,.woocommerce-page' ).on(
				'click', '.netmix_shop_mode_buttons a', function(e) {
					var mode = jQuery( this ).hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
					netmix_set_cookie( 'netmix_shop_mode', mode, 365 );
			...
			if (jQuery( '.woocommerce div.quantity .q_inc,.woocommerce-page div.quantity .q_inc' ).length == 0) {
				...
				jQuery( '.woocommerce div.quantity,.woocommerce-page div.quantity' ).on(
					'click', '>span', function(e) {
						netmix_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					}
				);
     ...
     // Inc/Dec quantity on buttons inc/dec
        function netmix_woocomerce_inc_dec_click(button) {
			...
			if (jQuery( 'table.cart' ).length > 0) {
				netmix_woocommerce_update_cart( 'init' );
			}
			...
			jQuery( window ).resize(
				function() {
					"use strict";
					if (jQuery( 'table.cart' ).length > 0) {
						netmix_woocommerce_update_cart( 'resize' );
					}
				}
			);
			...
			jQuery( document.body ).on(
				'updated_wc_div', function() {
					"use strict";
					if (jQuery( 'table.cart' ).length > 0) {
						netmix_woocommerce_update_cart( 'update' );
					}
				}
			);

			// Update cart display
			function netmix_woocommerce_update_cart(status){
				"use strict";
				setTimeout(
					function() {
						...
						if (NETMIX_STORAGE['mobile_layout_width'] < w && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
							var tbl = jQuery( 'table.cart' );
							if ( tbl.length > 0 ) {
