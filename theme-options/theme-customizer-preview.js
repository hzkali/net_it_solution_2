...
( function( $ ) {
	...
	var $style = $( '#netmix-customizer-inline-css' ),
		api    = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		var style_container = '<' + 'style type="text/css" id="netmix-customizer-inline-css" />',
			$theme_css      = $( '#netmix-custom-inline-css' );
		if ($theme_css.length == 0) {
			$theme_css = $( '#netmix-custom-css' );
		}
		if ($theme_css.length == 0) {
		...
		$style = $( '#netmix-customizer-inline-css' );
	}
			...
			api.preview.bind(
				'refresh-other-controls', function( obj ) {
					...
					if (id.indexOf( 'body_style' ) == 0) {
						...
						$( '.sc_layouts_logo .logo_text' ).html( netmix_prepare_macros( val ) );

					} else if (id == 'blogdescription') {
						$( '.sc_layouts_logo .logo_slogan' ).html( netmix_prepare_macros( val ) );

					} else if (id == 'copyright') {
						$( '.copyright_text' ).html( netmix_prepare_macros( val ) );
						...
						if (id == 'front_page_bg_image') {
							...
							if (color_type == 'none') {
								...
								color = typeof netmix_color_schemes[scheme] != 'undefined' ? netmix_color_schemes[scheme]['colors']['bg_color'] : '';
							}
							$( '.front_page_section_' + section + '_inner' ).css( 'background-color', color == '' ? 'transparent' : ( mask == 1 ? color : netmix_hex2rgba( color, mask ) ) );
							...
							.height( 'auto' )
							.toggleClass( 'netmix-full-height sc_layouts_flex sc_layouts_columns_middle', val );
							$( window ).trigger( 'resize' );
						}
					}
				}
